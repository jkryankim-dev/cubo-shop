...
## 핵심 도메인 사항
- **결제 수단**: **무통장입금(회사 법인계좌) 단일 운영**. PG(토스페이먼츠 등) 미사용.
- **결제 자격**: 사업자 회원 + 사업자등록증 `approved` 상태만 결제 가능 (비회원·일반 회원·미승인 사업자 모두 차단)
- **가격 노출**: 위 사업자 승인 회원에게만 표시. 그 외에는 ProductCard·상품 상세·장바구니 어디서도 가격 숨김
- **회사 법인계좌**: `/admin/settings` 에서 관리자가 직접 입력 → `shop_site_settings/payment` 에 저장 → 주문 시 스냅샷이 주문에 박힘
- **회원가입**: Firebase Auth (이메일 / 카카오·네이버 소셜은 추후)
- **배송**: 일반 택배 — 송장번호 입력형. **당일 출고는 오후 2시 이전 입금 확인(paid) 건만**
...
> 자주 저지르기 쉬운 LLM 실수를 줄이는 원칙. 자명한 작은 작업엔 판단껏, 그 외엔 반드시 따를 것.

### 1. 코딩 전에 생각 (Think Before Coding)
- **가정을 명시.** 불확실하면 코드 작성 전에 질문.
- 여러 해석이 가능하면 **모두 제시 후 사용자 선택** — 혼자 정하지 말 것.
- 더 단순한 길이 있으면 말할 것. 합리적 push back 환영.
- 헷갈리면 멈추고 무엇이 헷갈리는지 명시 → 질문.

### 2. 단순함 우선 (Simplicity First)
- 요청 외 기능·추상화·"유연성" 추가 금지.
- 일어날 수 없는 시나리오의 에러 처리 금지.
- 200줄로 쓴 게 50줄로 가능하면 다시 쓸 것.
- "선임 엔지니어가 과하다고 할까?" — 그렇다면 단순화.

### 3. 외과적 변경 (Surgical Changes)
- 요청과 무관한 인접 코드·주석·포맷팅 "개선" 금지.
- 기존 스타일에 맞출 것 (내가 다르게 했을 거라도).
- 내 변경이 만든 고아 (사용 안 되게 된 import/변수/함수) 만 제거.
- 기존부터 죽어있던 코드는 알리기만, 삭제 X.
- 테스트: **변경된 모든 줄이 사용자 요청에 직접 연결되는가?**

### 4. 목표 주도 실행 (Goal-Driven Execution)
- 검증 가능한 목표로 변환 ("X 추가" → "X 가 동작하는지 확인할 방법" 까지 포함).
- 다단계 작업은 짧은 계획 명시 후 진행: `1. [단계] → 검증: [확인]`
